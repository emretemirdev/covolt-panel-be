...
import com.covolt.backend.model.enums.CompanyStatus;
import com.covolt.backend.model.enums.CompanyType;
import jakarta.persistence.*;
import lombok.*; // Data, Builder, NoArgsConstructor, AllArgsConstructor, EqualsAndHashCode

import java.util.ArrayList;
import java.util.List;
import java.util.HashSet; // Set için eklendi
import java.util.Set;     // Set için eklendi
...
@Table(name = "companies", indexes = { // Arama performansı için indexler
        @Index(name = "idx_companies_name", columnList = "name", unique = true) // Firma adı genellikle benzersiz olmalı
})
public class Company extends BaseEntity { // BaseEntity'den miras alıyor (id, createdAt, updatedAt, version)

    @Column(nullable = false) // Firma adı zorunlu
    private String name;
